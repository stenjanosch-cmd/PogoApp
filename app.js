...
function setupCheatsheet() {
    ...
    Object.keys(typeTranslations).forEach(type => {
        let img = document.createElement("img"); img.className = "cheat-type-icon"; img.src = `https://raw.githubusercontent.com/duiker101/pokemon-type-svg-icons/master/icons/${type}.svg`;
        img.style.backgroundColor = typeColors[type]; img.onclick = () => renderCheatResult(type); grid.appendChild(img);
    });
}
